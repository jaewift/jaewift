    ...
    <Reveal className="w-full md:w-auto lg:sticky lg:top-50">
      <div className="w-full flex flex-col items-end gap-4 sm:gap-5 md:gap-6 lg:gap-7 xl:gap-8 mt-0 sm:mt-4 md:mt-6 lg:mt-10 xl:mt-12">
        ...
        <div className="w-full relative flex flex-col gap-12 sm:gap-14 md:gap-16 lg:gap-20 xl:gap-24">
          <div className="absolute right-2 sm:right-2.5 md:right-3 lg:right-3.5 xl:right-4 top-3 sm:top-4 md:top-5 xl:top-6 bottom-3 sm:bottom-4 md:bottom-5 xl:bottom-6 w-0.5 bg-gradient-to-b from-primary/70 via-primary/30 to-border"></div>
          {educations.reverse().map((education, index) => (
            <div
              ...
              <div className="text-[10px] sm:text-xs text-primary font-medium bg-primary/10 border border-primary/25 px-2 py-0.5 sm:px-3 md:py-1 lg:px-4 rounded-full">
              ...
              <div className="absolute right-0 top-1.5 sm:top-2 md:top-2.5 xl:top-3 w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 lg:w-7 lg:h-7 xl:w-8 xl:h-8 rounded-full bg-primary border-2 sm:border-3 md:border-3 lg:border-4 border-background shadow-[0_0_16px_-2px_var(--theme-color-primary)]"></div>
            </div>
          ))}
        </div>
      </div>
